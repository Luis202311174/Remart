  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        ...
        // ✅ Simpler query (no invalid joins)
        const { data: chatsData, error } = await supabase
          .from("chats")
          .select("chat_id,buyer_auth_id,seller_auth_id,product_id,created_at")
          .or(`buyer_auth_id.eq.${auth.user.id},seller_auth_id.eq.${auth.user.id}`)
          .order("created_at", { ascending: false });

        if (error) throw error;

        // ✅ Collect all involved user IDs to fetch emails
        const userIds = [
          ...new Set(
            chatsData.flatMap(c => [c.buyer_auth_id, c.seller_auth_id])
          ),
        ].filter(Boolean);
        ...
        // ✅ Handle chatTarget (coming from product page)
