...
function formatProduct(p) {
  ...
  // ✅ LOG raw lat/lng
  console.log("formatProduct -> raw lat/lng:", p.lat, p.lng);
...
async function mergeProfiles(products, supabaseClient) {
  ...
  return products.map((p) => {
    ...
    // ✅ Preserve lat/lng and all top-level product fields
...
/* -----------------------------------------------------
   🔹 Fetch all products (excluding logged-in seller & sold)
----------------------------------------------------- */
export async function fetchAllProducts(limit = 20, context = null) {
  try {
    ...
    console.log("fetchAllProducts -> raw data:", data);
...
export async function fetchProductById(id, context = null) {
  try {
    ...
    console.log("fetchProductById -> raw product:", product);
...
/* -----------------------------------------------------
   🔹 Fetch similar products by category (exclude sold)
----------------------------------------------------- */
export async function fetchSimilarProducts(catId, excludeId, limit = 4, context = null) {
  try {
    ...
    console.log("fetchSimilarProducts -> raw data:", data);
...
/* -----------------------------------------------------
   🔹 Fetch products with filters (exclude sold & own seller)
----------------------------------------------------- */
export async function fetchFilteredProducts({
  ...
  try {
    ...
    console.log("fetchFilteredProducts -> raw data:", data);
